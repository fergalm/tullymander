...
#This is how I added dates to a coloubar's labels.
#I haven' figured out how to make this a function yeat.
# import matplotlib.dates as mdates
# dt = pd.to_datetime(t)
# mpl_date = mdates.date2num(dt)
# plt.scatter(x, y, s=80, c=mpl_date, cmap=cmap)
# cb = plt.colorbar()
...
def scene_coverage(lngBins, latBins, scene_list, **kwargs):
    """Compute and plot the number of scenes that cover each part of an image

    Inputs:
    -------------
    lngBins, latBins
        (arrays) Locations of bottom left hand corner of bins
    scene_list
        (iterable) List of scenes bounds. Each bound may be an OGR object
        or a WKT string
    ...
    -----------------
    steps_for_cmap
        (int or string) Colormap used for indicating scene density in each
        grid cell is discretized. By default there are 8 steps, but the
        number can be controlled with this argument. If set to 'int', the
        steps are chosen as integers if the range of cell values is less
        than 8.
    Passed to ``matplotlib.pyplot.imshow()``
    ...
    * Assumes the earth is flat
    ...
    #Prep
    coverage = np.zeros( (len(latBins), len(lngBins)) )
    if isinstance(list(scene_list)[0], str):
        scene_list = map(lambda x: ogr.CreateGeometryFromWkt(x), scene_list)

    #Compute coverage
    offset_lng = .5 * (lngBins[1] - lngBins[0])
    offset_lat = .5 * (latBins[1] - latBins[0])
    for col, lng in enumerate(lngBins):
        for row, lat in enumerate(latBins):
            p = ogr.Geometry(type=ogr.wkbPoint)
            p.AddPoint(lng + offset_lng, lat + offset_lat)
            coverage[row, col] = \
                np.sum( map(lambda x: x.Contains(p), scene_list))
...
#    cmap.set_under('')
    ...
    vmax = kwargs.pop('vmax', np.max(coverage))
    steps = kwargs.pop('steps_for_cmap', 8)

    if steps == 'int':
        steps = min( vmax - vmin + 1, 8)


    extent = [np.min(lngBins), np.max(lngBins), \
              np.min(latBins), np.max(latBins)]

    import matplotlib.colors as mcolors
    norm = mcolors.BoundaryNorm(np.linspace(vmin, vmax, steps), cmap.N)
    plt.imshow(coverage, extent=extent, cmap=cmap,

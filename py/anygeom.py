...
class AnyGeom(object):
    ...
    * ofl is the format used by `OFL.base.utils.shapefiles.save_geometries`
    * tiles are defined in `OFL.base.utils.tiler`
    """
    ...
    def as_ofl(self):
        """Convert to a geomery/dict tuple as used by OFL

        The module ``OFL.base.utils.shapefiles`` has two popular functions
        ``get_geometries`` and ``save_geometries``. They operate on a list
        of tuples, where each tuple contains a geometry and a dictionary
        of metadata data. This function returns the AnyGeom in that format.
        """
        return self.as_geometry(), self.meta


    def as_tilelist(self, tilespec):
        try:
            from OFL.base.utils.tile_util import TilingSpecification
        except ImportError:
            raise ImportError("Failed to import OFL.base.utils.tile_util")

        tiler = TilingSpecification(tilespec)
        tilelist = tiler.get_tiles(self.obj, precise=True)
        return tilelist
